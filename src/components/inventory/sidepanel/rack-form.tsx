...
export function RackForm({ item }: RackFormProps) {
  const { updateItem, removeItem, rotateSelected } = useZoneStore();
  ...
  return (
    <div className="space-y-4">
          ...
          {Array.from({ length: item.floors }, (_, floorIndex) => (
            <div key={floorIndex} className="flex flex-col">
              ...
              <Input
                ...
                type="number"
                min="0"
                value={item.floorCapacities?.[floorIndex] || ''}
                onChange={(e) => {
                  const newCapacities = [...(item.floorCapacities || Array(item.floors).fill(0))];
                  newCapacities[floorIndex] = parseInt(e.target.value) || 0;
                  handleUpdate({ floorCapacities: newCapacities });
                }}
              />
            </div>
          ))}
      ...
      <div>
        <Label htmlFor="totalMaxCapacity">Total Max Capacity</Label>
        <Input
          id="totalMaxCapacity"
          type="number"
