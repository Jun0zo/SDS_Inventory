-- Migration: Fix Rack Location Regex Pattern for Single-Letter Identifiers
-- Date: 2025-12-17
-- Purpose: Fix broken location matching for single-letter rack identifiers (B, C, etc.)
--
-- Problem: The regex pattern '-[0-9]+-[0-9]+$' requires digits immediately after
--          the location identifier, which fails for single-letter locations.
--          Works: A16-01-01 (A16 has digits before suffix)
--          Fails: B-01-01 (B has no digits, goes straight to suffix)
--
-- Solution: Make the suffix optional: '(-[0-9]+-[0-9]+)?$'
--          This pattern allows matching both:
--          - B (exact location match)
--          - B-01-01 (location with cell coordinates)
--
-- Affected Views:
-- ✅ location_inventory_summary_mv (3 pattern fixes)
-- ✅ item_inventory_summary_mv (2 pattern fixes)
--
-- Changes:
-- Line 63, 107, 188: location_inventory_summary_mv pattern fixes
-- Line 300, 375: item_inventory_summary_mv pattern fixes
...
DO $$
BEGIN
  RAISE NOTICE 'Recreating location_inventory_summary_mv with fixed rack location pattern...';
END $$;
  ...
  LEFT JOIN (
    ...
    JOIN public.wms_raw_rows w ON
      ...
      AND (
        ...
        OR
        -- Rack items: pattern match (A35 matches A35-01-01, B matches B-01-01)
...
LEFT JOIN public.wms_raw_rows w ON
  ...
  AND (
    ...
    OR
    -- Rack items: pattern match (e.g., A35 matches A35-01-01, B matches B-01-01)
...
COMMENT ON MATERIALIZED VIEW public.location_inventory_summary_mv IS
  'Pre-aggregated inventory summaries by ITEM (not WMS location) for Zone Layout Editor SidePanel and Dashboard heatmap.
   ...
   - current_stock_count: COUNT of unique WMS locations (e.g., for rack "A35": counts A35-01-01, A35-01-02, A35-02-01 as 3)
   ...
   - Rack items: pattern match (e.g., "A35" matches "A35-01-01", "B" matches "B-01-01" using regex with optional suffix)
   ...
   - This view adds current_stock_count = COUNT(DISTINCT wms_locations)
...
DO $$
BEGIN
  RAISE NOTICE 'Recreating item_inventory_summary_mv with fixed rack location pattern...';
END $$;
  ...
  LEFT JOIN (
    ...
    JOIN public.wms_raw_rows w ON
      ...
      AND (
        ...
        OR
        -- Rack 아이템: base location + 숫자-숫자 패턴 매칭 (예: A35-01-01, B-01-01)
...
COMMENT ON MATERIALIZED VIEW public.item_inventory_summary_mv IS
  'Pre-calculated inventory for each layout component (items table).
   ...
   - Zone + Location simultaneous matching:
     * Flat items: exact location match (e.g., B1 = B1)
     * Rack items: prefix pattern match (e.g., A35 matches A35-01-01, B matches B-01-01)
...
DECLARE
  ...
  v_b_rack_count INTEGER;
  v_b_rack_with_data INTEGER;
BEGIN
  RAISE NOTICE '========================================';
  ...
  -- Check how many items have matched WMS data (current_stock_count > 0)
  SELECT COUNT(*) INTO v_items_with_data
  FROM public.location_inventory_summary_mv
  WHERE current_stock_count > 0;
  RAISE NOTICE 'Items with WMS data matched (current_stock_count > 0): %', v_items_with_data;

  -- Check B rack items specifically
  SELECT COUNT(*) INTO v_b_rack_count
  FROM public.location_inventory_summary_mv
  WHERE item_location LIKE 'B%' AND type = 'rack';
  RAISE NOTICE 'Total B rack items: %', v_b_rack_count;

  SELECT COUNT(*) INTO v_b_rack_with_data
  FROM public.location_inventory_summary_mv
  WHERE item_location LIKE 'B%' AND type = 'rack' AND current_stock_count > 0;
  RAISE NOTICE 'B rack items with inventory data: %', v_b_rack_with_data;
  ...
  IF v_location_mv_count > 0 AND v_item_mv_count > 0 THEN
    ...
    END IF;
    IF v_b_rack_count > 0 THEN
      IF v_b_rack_with_data > 0 THEN
        RAISE NOTICE '✅ B rack location matching is working (%/% B racks have data)', v_b_rack_with_data, v_b_rack_count;
      ELSE
        RAISE WARNING '⚠️  B racks exist but none have inventory data. Check WMS data.';
      END IF;
    ELSE
      RAISE NOTICE 'ℹ️  No B rack items found in this layout.';
    END IF;
  ELSE
...
-- Uncomment to see B rack data from location_inventory_summary_mv
/*
SELECT
  item_id,
  warehouse_code,
